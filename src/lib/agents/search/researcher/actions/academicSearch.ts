...
const academicSearchAction: ResearchAction<typeof schema> = {
  ...
  getToolDescription: (config) => {
    ...
    return `Use this tool to perform academic searches for scholarly articles, papers, and research studies relevant to the user's query. You can provide up to ${limit} queries at a time. Provide a list of concise search queries that will help gather comprehensive academic information on the topic at hand.`;
  },
  enabled: (config) =>
