...
const academicSearchAction: ResearchAction<typeof schema> = {
  ...
  getToolDescription: (config) => {
    ...
    return `Use this tool to perform academic searches for scholarly articles, papers, and research studies relevant to the user's query. Provide a list of concise search queries (up to ${limit} at a time) that will help gather comprehensive academic information on the topic at hand.`;
  },
  enabled: (config) =>
